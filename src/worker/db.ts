...
export async function restRequest<T>(env: DbEnv, path: string, init: RequestInit = {}): Promise<T> {
...
export async function restSelect<T>(env: DbEnv, path: string): Promise<T[]> {
  return restRequest<T[]>(env, path, { method: "GET" });
}

export async function restInsert<T>(env: DbEnv, table: string, row: Record<string, unknown>): Promise<T> {
...
export async function restUpdate<T>(env: DbEnv, table: string, filter: string, patch: Record<string, unknown>): Promise<T[]> {
